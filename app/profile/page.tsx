...
export default function ProfilePage() {
  ...
  const [generatedInfo, setGeneratedInfo] = useState({ address: '', privateKey: '' })

  const handleGenerateWallet = async () => {
    try {
      setIsUpdating(true)
      const response = await authApi.post('/user/create-wallet')
      if (response.data) {
        const { walletAddress: newAddr, walletPrivateKey } = response.data
        setGeneratedInfo({ address: newAddr, privateKey: walletPrivateKey })
                        ...
                        <>
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between items-center">
                                ...
                                <button 
                                  ...
                                  className="text-[10px] font-bold text-yellow-500 hover:text-yellow-400 transition-colors uppercase tracking-wider"
                                >
                                  Generate Inbuilt Wallet
                                </button>
                              </div>
                              <input
                                type="text"
                          ...
                          {showWalletModal && (
                            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                              <div className="bg-[#1a1a1a] border border-yellow-500/20 p-8 rounded-3xl max-w-lg w-full shadow-2xl relative overflow-hidden group">
                                ...
                                <div className="flex flex-col items-center text-center space-y-6">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <p className="text-gray-400 text-sm">Please save your private key safely. It will not be shown again.</p>
                                  </div>

                                  <div className="w-full space-y-4 text-left">
                                    <div className="p-4 rounded-2xl bg-black/40 border border-white/5 space-y-1">
                                      <label className="text-[10px] font-bold text-yellow-500 uppercase tracking-widest">Public Address</label>
                                      <p className="text-xs text-white break-all font-mono">{generatedInfo.address}</p>
                                    </div>
                                    <div className="p-4 rounded-2xl bg-yellow-500/5 border border-yellow-500/10 space-y-1">
                                      <label className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Private Key (SECRET)</label>
                                      <p className="text-xs text-yellow-500 break-all font-mono">{generatedInfo.privateKey}</p>
                                    </div>
                                  </div>

                                  <button 
                                    onClick={() => setShowWalletModal(false)}
                                    className="w-full btn-gold py-4 rounded-2xl font-bold shadow-lg shadow-yellow-500/20"
                                  >
